...
class den_stream:
	...
	def get_cluster_result(self):
		...
		return labels, clustered_points, bounding_boxes, np.array(core_centroids)

	def validate(self, ob_locs):

		labels, cluster_points, bounding_boxes, centroids = self.get_cluster_result()
...
data, ego_pos, object_locs = load_pkl(frame_no, radius)
